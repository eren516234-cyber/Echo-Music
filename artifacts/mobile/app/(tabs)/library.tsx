...
import { SONGS, ALBUMS, ARTISTS, PLAYLISTS, fmt } from "@/constants/data";
...
export default function LibraryScreen() {
  ...
  const { playSong, currentSong, liked, toggleLike } = usePlayer();
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 27 : 0) }]}>
      ...
      <FlatList
        ...
        contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 160, gap: activeTab === "Artists" ? 0 : 4 }}
        ...
        renderItem={({ item }) => {
            ...
            return (
              ...
              >
                ...
                <View style={styles.songInfo}>
                  <Text style={[styles.songTitle, isActive && { color: accent }]} numberOfLines={1}>{song.title}</Text>
                  <Text style={styles.songSub} numberOfLines={1}>{song.artist}</Text>
                </View>
                <Text style={styles.songDur}>{fmt(song.duration)}</Text>
                <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); toggleLike(song.id); }}>
                  <Feather name="heart" size={15} color={liked.has(song.id) ? accent : "#333"} />
                </Pressable>
              </Pressable>
            );
          }
          ...
          if (activeTab === "Albums") {
            ...
            return (
              <Pressable style={styles.albumRow}>
                ...
                <View style={styles.songInfo}>
                  <Text style={styles.songTitle} numberOfLines={1}>{album.title}</Text>
                  <Text style={styles.songSub}>{album.artist} · {album.year}</Text>
                </View>
          ...
          if (activeTab === "Artists") {
            ...
            return (
              <Pressable style={styles.artistCard}>
                ...
                <Text style={styles.artistSub} numberOfLines={1}>{artist.genre}</Text>
              </Pressable>
            );
          }
          ...
          return (
            <Pressable style={styles.playlistRow}>
              <Image source={{ uri: pl.artwork }} style={styles.albumArt} />
              <View style={styles.songInfo}>
                <Text style={styles.songTitle} numberOfLines={1}>{pl.title}</Text>
                <Text style={styles.songSub}>{pl.songCount} songs</Text>
              </View>
              <Feather name="chevron-right" size={16} color="#333" />
            </Pressable>
          );
...
const styles = StyleSheet.create({
  ...
  songRow: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 8, paddingHorizontal: 8, borderRadius: 10 },
  ...
  songSub: { fontSize: 12, color: "#666", fontFamily: "Inter_400Regular", marginTop: 2 },
  songDur: { fontSize: 12, color: "#444", fontFamily: "Inter_400Regular" },
  albumRow: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 8, paddingHorizontal: 8 },
  albumArt: { width: 52, height: 52, borderRadius: 8, backgroundColor: "#1a1a1a" },
  playlistRow: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 8, paddingHorizontal: 8 },
